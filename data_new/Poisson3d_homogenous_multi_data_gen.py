...
## Boundary data
perms =  ["".join(seq) for seq in itertools.product("01", repeat=num_dim)]
    ...
    for j in range(num_dim):
        for p in range(len_perm):
            ...
            for i in range(train_size_b):
                ...
                for d in range(num_dim):
                    if j == d: 
                        train_data_b[j*(num_dim)+p][l][i][d] = bound_rand_data[j*(num_dim)+p][l][k][0]
                    else:
                        train_data_b[j*(num_dim)+p][l][i][d] = int(perms[p][cc])
                        cc = cc +1
            k = 0
            for i in range(train_size_b):
                for d in range(num_dim):
...
filename = './poisson_homogenous' + str(num_dim) + 'D_no_boundary_'  + '.mat' # './poisson4D_multi_[10-100].mat'
# filename = 'sims.mat'
