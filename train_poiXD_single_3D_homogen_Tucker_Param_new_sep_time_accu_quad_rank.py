...
net_config_outer_arr1.append([qx_dim, 32,32,r])
net_config_outer_arr1.append([qx_dim, 64,64,r])
#net_config_outer_arr1.append([qx_dim, 64,64,64,r1*r])
#net_config_outer_arr1.append([qx_dim, 128,128 ,r1*r])
#net_config_outer_arr1.append([qx_dim, 128,128,r1*r])


net_config_core_arr_last = list()
net_config_core_arr_last.append([])
...
for net_config_X in net_config_X_arr:
    for net_config_outer1 in net_config_outer_arr1:
        for net_config_core_arr_ in net_config_core_arr_last:
            for wd_ in wd:
                for drop_ in drop:
                                    ...
                                    for quad_ in quads:
                                        ...
                                        for it in range(no_samples):
                                            ...
                                            def train():
                                                ...
                                                #net_config_core_arr_[0] = rank2_
                                                #net_config_core_arr_[-1] = (rank2_**3)
                                                mlp_quad_last_shape[0] = rank_
                                                mlp_quad_last_shape[1] = rank2_
                                                mlp_quad_last_shape[2] = rank2_
                                                mlp_quad_last_shape[3] = rank2_
                                                
                                                mlp_dims = [rank_,rank2_,rank2_,rank2_]
                                                ...
                                                mlp_quad1= nn_mlp_last_na_custom_act_tensor(net_config_outer1, mlp_quad_act1, mlp_quad_last_shape[1] ).to(device)
                                                nn_quad_list.append(mlp_quad1)
                                                
                                                mlp_quad_act1 = activation_ext(act = args.act_type_quad).to(device)
                                                mlp_quad2= nn_mlp_last_na_custom_act_tensor(net_config_outer1, mlp_quad_act1, mlp_quad_last_shape[2] ).to(device)
                                                nn_quad_list.append(mlp_quad2)
                                                
                                                mlp_quad_act1 = activation_ext(act = args.act_type_quad).to(device)
                                                mlp_quad3= nn_mlp_last_na_custom_act_tensor(net_config_outer1, mlp_quad_act1, mlp_quad_last_shape[3] ).to(device)
                                                ...
                                                model = DecGreenNet_product_tucker_param_3(mlp_x,  r_func ,nn_quad_list,  mlp_quad_last_shape,quad_concat , mlp_dims ).to(device)
                                            ...
                                            filename = './Poisson3D_gridsearch_homogen_tucker/param/'+str(args.data) + '_' + str(args.seed)   + '_' + str(lr)     + '_' + str(wd)  + '_' + str(drop)   + '_' + str(reg_p1)   + '_' + str(reg_p2)   + '_' + str(net_config_X)   + '_'   + str(net_config_outer1)     + '_'   + str(net_config_core_arr_)   + '_'   + str(rank_)  + '_'   + str(rank2_)  + '_'   + str(quad_)  + '_' + str(args.act_type_x) + '_' + str(args.act_type_quad)   + '_' + str(args.act_type_last)  + '_' + str(loss_val)  + '_' + str(error_abs)  + '_time_' + str(t)  + '.mat'
